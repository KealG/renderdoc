...
The first step is to build |brand_product_name| from source to ensure you have a matching python module and |brand_product_name| build. Due to binary incompatibilities the python module can't be widely distributed since it is linked against a specific python minor version.

Build instructions for your platform are available `on github <|brand_source_url|>`_. Take note of the python version that you build against, since this will need to be used later.
...
------------------------
Using the same python version as the |brand_product_name| build used (Python 3.6 by default).
...
After running the command the output folder `<path_to_folder_to_store_render_python_symbols>` should contain |brand_python_core_module_code| and |brand_python_gui_module_code| folders.
...
Now when viewing a python script in `VS Code`, you should see the |brand_product_name| types being correctly resolved and autocompleted.

If you get a warning about failure to import module |brand_python_core_module_code| then something is not correct in the setup, double check the `VS Code` setting `python.analysis.extraPaths` includes the |brand_product_name| python API parsed output folder (which should contain two folders |brand_python_core_module_code| and |brand_python_gui_module_code|).
...
Before you run PyCharm, we will replace one file in it to generate better type information for |brand_product_name|. In the |brand_product_name| repository there's a `pycharm_helpers folder <|brand_pycharm_helpers_url|>`_. Copying the content of the plugins folder over the folder in your PyCharm installation will update the file that is customised. You can back it up beforehand at this path: ``plugins/python-ce/helpers/generator3/module_redeclarator.py``.
...
Once you've chosen the correct interpreter we'll also tell it where to find the |brand_product_name| libraries since they won't be in the default python path. Go back to the gear wheel but this time select :guilabel:`Show All`. With your chosen interpreter selected click on the tree icon at the bottom labeled ``Show paths for the selected interpreter`` and add the directory where you have the |brand_python_core_module_code| and |brand_python_gui_module_code| modules, as well as the |brand_python_core_module_code| library.

If everything went well, PyCharm should load that interpreter for the project and discover the |brand_python_core_module_code| and |brand_python_gui_module_code| modules. It will then generate stubs for them with correct typing information so you can benefit from proper autocomplete while writing python code.
