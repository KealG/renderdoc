...
The python shell allows flexible use of |brand_product_name|, including the ability to run simple scripts over the whole dataset in ways not easy to do in the UI. Depending on the build-time configuration the Qt UI itself may also be exposed to python for customisation.
...
To get started the :code:`pyrenderdoc` object corresponds to a |brand_python_gui_capturecontext_code| object through which the internal API and UI windows can be obtained.
