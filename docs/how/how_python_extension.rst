...
|brand_product_name| supports python extensions using the :doc:`existing python API <../python_api/index>`. These extensions can be configured to be persistent and load whenever the UI does, allowing users to extend the program with custom functionality.

Example extensions can be found at the `community contributed repository <https://github.com/ripperk/ripperk-contrib>`_. More information on writing an extension is available at :doc:`../python_api/ui_extensions`.
